...
from cellarium.cas_backend.core.db.field_type_decorators import SQLiteCompatibleUUIDType

# The current version of the token
CURRENT_TOKEN_VERSION: str = "2"


def _default_user_key_expiration() -> datetime.datetime:
    return datetime.datetime.now(datetime.UTC) + datetime.timedelta(seconds=settings.JWT_DEFAULT_TOKEN_TTL)
...
class User(db.Base):
    ...
    created_at = sa.Column(sa.DateTime, nullable=False, default=(lambda: datetime.datetime.now(datetime.UTC)))
...
class UserActivity(db.Base):
    ...
    finished_time = sa.Column(sa.DateTime, default=(lambda: datetime.datetime.now(datetime.UTC)))
...
sa.inspect(User).add_property(
    ...
    prop=column_property(
        ...
        + sa.select(sa.func.coalesce(sa.func.sum(UserActivity.cell_count), 0))
...
sa.inspect(User).add_property(
    ...
    prop=column_property(
        ...
        + sa.select(sa.func.coalesce(sa.func.count(UserActivity.id), 0))
        .where((UserActivity.user_id == User.id) & (UserActivity.event == UserActivityEvent.SUCCEEDED))
        .scalar_subquery()
    ),
)
...
class UserKey(db.Base):
    ...
    created_date = sa.Column(sa.DateTime, default=(lambda: datetime.datetime.now(datetime.UTC)), nullable=False)
    active = sa.Column(sa.Boolean(), default=True, nullable=False)
    expires = sa.Column(
        sa.DateTime,
        default=_default_user_key_expiration,
        nullable=False,
    )
