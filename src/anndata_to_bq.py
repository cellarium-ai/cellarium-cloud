...
def process(input_file, cas_cell_index_start, cas_gene_index_start):
    ...
    for col in range(0, len(adata.var)):
        col_index_to_cas_gene_index[col] = adata.var['cas_gene_index'].iloc[[col]][0]
    
    adata.var[['cas_gene_index', 'original_gene_id', 'feature_name']].to_csv(
        'cas_gene_info.tsv.gz', index=False, sep='\t', compression='gzip')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cas_cell_index_start', type=int, help='starting number for cell index', required=False,
                        default=0)
    parser.add_argument('--cas_gene_index_start', type=int, help='starting number for gene index', required=False,
                        default=0)
