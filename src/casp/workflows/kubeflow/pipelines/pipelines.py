...
@dsl.pipeline(name="pca_resize_full_cycle_in_parallel", description="Incremental PCA Resize ")
...
@dsl.pipeline(name="pca_embed_index_create", description="PCA Embed Create Index")
def pca_embed_and_index_create_pipeline(pca_embed_and_index_create_pipeline_config_paths: t.List[str]):
    with dsl.ParallelFor(pca_embed_and_index_create_pipeline_config_paths, parallelism=3) as item:
        embed_job_task_op = create_job(
            component_func=job_components.pca.embed,
            component_name=constants.PCA_EMBED_COMPONENT_NAME,
            gcs_config_path=item.pca_embed_gcs_config_path,
