...
import pytest
import re
from mockito import when, mock, unstub, verify, matchers, ANY
import numpy as np
...
INDEX = models.CASMatchingEngineIndex(
    is_grpc=True, endpoint_id="endpoint_id_grpc", deployed_index_id="deployed_index_id_grpc", num_neighbors=3, model_id=1)
...
class TestCellOperationsService:
    ...
    def setup_method(self) -> None:
        self.cell_operations_service = CellOperationsService(
            cell_operations_dm=mock(),
            cellarium_general_dm=mock()
        )
    ...
    def test_non_admin_can_not_access_admin_model(self) -> None:
        ...
        with pytest.raises(exceptions.AccessDeniedError, match=re.escape('admin_only_model model is not available.')):
            self.cell_operations_service.authorize_model_for_user(
                user=USER_NON_ADMIN,
                model_name="admin_only_model"
            )

    def test_model_does_not_exist(self) -> None:
        when(self.cell_operations_service.cellarium_general_dm).get_model_by_name(
            model_name="non_existent_model").thenRaise(dm_exc.NotFound())
    ...
    def test_mismatched_embeddings_and_queries(self) -> None:
        ...
        with pytest.raises(exceptions.VectorSearchResponseError, match=re.escape('Number of query ids (1) and knn matches (0) does not match.')):
    ...
    def test_empty_neighbors(self) -> None:
        ...
        with pytest.raises(exceptions.VectorSearchResponseError, match=re.escape('Vector Search returned a match with 0 neighbors.')):
            CellOperationsService._CellOperationsService__validate_knn_response(
                embeddings=embeddings, knn_response=knn_response)
    ...
    @parameterized.expand([
        ...
        ([
            ...
            ["erythrocyte", "monocyte"], True),
    ])
    @pytest.mark.asyncio
    async def test_annotate_adata_file(self, embeddings: List[List[float]], cell_types: List[str], include_dev_metadata: bool) -> None:
        """
        ...
        response = [{
            ...
            "matches": [{"cell_type": cell_types[i], "cell_count": 10}]
        ...
        actual_response = await self.cell_operations_service.annotate_adata_file(
            user=USER_ADMIN, file=io.BytesIO(ANNDATA_DATA), model_name=MODEL.model_name, include_dev_metadata=include_dev_metadata)
    ...
    @parameterized.expand([
        ...
        ([
            ...
            },
            {
                "query_cell_id": "q1",
                "neighbors": [
                    {"cas_cell_index": "1", "distance": 0.0}
                ]
            }
        ]),
    ])
    @pytest.mark.asyncio
    async def test_search_adata_file(self, embeddings: List[List[float]], expected_response: List[Dict[str, Any]]) -> None:
        """
    ...
    async def test_get_cells_by_ids_for_user(self) -> None:
        ...
        self.cell_operations_service.get_cells_by_ids_for_user(
            user=USER_ADMIN, cell_ids=cell_ids, model_name=MODEL.model_name, metadata_feature_names=metadata_feature_names
        )
    ...
    async def test_get_cells_by_ids_for_user_bad_feature_name(self) -> None:
        ...
        with pytest.raises(exceptions.CellMetadataColumnDoesntExist, match=re.escape(f"Feature foo is not available for querying.")):
            self.cell_operations_service.get_cells_by_ids_for_user(
                user=USER_ADMIN, cell_ids=cell_ids, model_name=MODEL.model_name, metadata_feature_names=metadata_feature_names
            )
    ...
    def __mock_apis(
        ...
        when(clients.ModelInferenceClient).call_model_embed(file_to_embed=anndata_data, model_name=model_name).thenReturn(
            async_return({"obs_ids": query_ids, "embeddings_b64": utils.numpy_to_base64(embeddings)}))

        when(self.cell_operations_service.cellarium_general_dm).get_model_by_name(
            model_name=model_name).thenReturn(model)
        when(self.cell_operations_service.cellarium_general_dm).get_index_for_model(
            model_name=model_name).thenReturn(index)

        # mock calls to that use the matching client
        matching_client = mock()
        matching_client_response = MatchResult(matches=[
            MatchResult.NearestNeighbors(
                neighbors=[MatchResult.Neighbor(id=str(i), distance=0.0, feature_vector=embedding)])
            for i, embedding in enumerate(embeddings)
        ]) if matching_client_response is None else matching_client_response

        # Note: comparing numpy arrays with == doesn't work well with mockito so we need to be a little clever here
        when(matching_client).match(queries=matchers.arg_that(
            lambda arg: np.array_equal(arg, embeddings))).thenReturn(matching_client_response)
