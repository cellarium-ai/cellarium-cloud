...
from casp.services.api import data_manager
...
class MockCellQuotaDataManager(data_manager.CellQuotaDataManager):
    ...
    def __init__(self):
        ...
        # self.system_data_db_session_maker = MagicMock(return_value=self.mock_session)
...
class MockMatchingClient(AsyncMock):
    ...
    def __init__(self, cell_source: t.Dict[int, t.Any], *args, **kwargs) -> None:
        """
        ...
        :param cell_source: A dictionary representing the centralized cell resource where keys are cell indices and
    ...
    async def _match_side_effect(self, queries: t.List[t.Any]) -> MatchResult:
        ...
        """
        matches = []
        cell_indices = list(self.cell_source.keys())
